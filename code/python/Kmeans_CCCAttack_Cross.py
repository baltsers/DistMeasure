...
#pd.read_csv('C:/Research/distMeasureML12/RMCCCCAttack5065.csv')
...
# dt.to_csv("C:/Research/distMeasureML12/RMCCCC5065_KMSplit.csv", index=0)
  ...
  CCCAttack=Z.reset_index(drop=True)   # data5065
  label= pd.DataFrame(clf.labels_)
  print ("CCCAttack.columns.size=",CCCAttack.columns.size," CCCAttack.iloc[:,0].size=",CCCAttack.iloc[:,0].size)
  print ("CCCAttack=", CCCAttack)
  print ("label.columns.size=",label.columns.size," label.iloc[:,0].size=",label.iloc[:,0].size)
  print ("label=", label) 

  CCCAttack_label=pd.concat([CCCAttack, label], axis=1, ignore_index=True)
  #CCCAttack_label=CCCAttack.append(label, ignore_index=True)
# Attack_label=CCCAttack_label.rename(columns={'0':'label'}, inplace = True)
  print ("CCCAttack_label.columns.size=",CCCAttack_label.columns.size," CCCAttack_label.iloc[:,0].size=",CCCAttack_label.iloc[:,0].size)
  print ("CCCAttack_label=", CCCAttack_label)
...
#print (sum2)
  ...
  CCCAttack5000=W.reset_index(drop=True)
  print ("CCCAttack5000.columns.size=",CCCAttack5000.columns.size," CCCAttack5000.iloc[:,0].size=",CCCAttack5000.iloc[:,0].size)
  print ("CCCAttack5000=", CCCAttack5000) 
  #pd.read_csv('C:/Research/distMeasureML12/CCCAttack5065.csv')
  diff1=abs(CCCAttack5000["Attack"]-average1)
  print ("diff1=",diff1)

  diff2=abs(CCCAttack5000["Attack"]-average2)
  print ("diff2=",diff2)
  ...
  #CCCAttack5000.columns = ['RMC','CCC','Attack','D0','D1','ClosedTo','Predicted']
  #print (CCCAttack5000)
...
def cvOneTime(fileName,k,n):
  ...
  dataTrain1 = pd.DataFrame(columns=['CCC', 'Attack'])
  startTrain1=0;
  ...
  dataTrain2 = pd.DataFrame(columns=['CCC', 'Attack'])
...
if __name__=="__main__":
  for num in range(0,9):
    cvOneTime('C:/Research/distMeasureML12/CCCAttack16882.csv',10,num)
