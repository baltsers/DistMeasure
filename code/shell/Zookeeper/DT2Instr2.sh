...
SOOTCP=".:/usr/lib/jvm/java-8-openjdk-amd64/jre/lib/rt.jar:$subjectloc/build2/test/classes:/home/xqfu/DistTaint/bin"


echo $SOOTCP
OUTDIR=$subjectloc/DT2Instr2
rm -R $OUTDIR -f
mkdir -p $OUTDIR
...
java -Xmx100g -ea -cp ${MAINCP} disttaint.dt2Inst \
	...
	-process-dir $subjectloc/build2/test/classes \
	-allowphantom \
	-wrapTryCatch \
    -intraCD \
    -interCD \
    -exInterCD \
	# -main-class org.apache.zookeeper.test.system.InstanceContainer -entry: org.apache.zookeeper.test.system.InstanceContainer \
