...
INDIR=$subjectloc/test1_Integration
...
MAINCP=".:/opt/jdk1.8.0_101/jre/lib/rt.jar:$ROOT/DUAForensics.jar:$ROOT/distEADIVER.jar:$ROOT/libs/soot-trunk.jar:$subjectloc/bin"
...
starttime=`date +%s%N | cut -b1-13`
	...
	#-debug
	#"-nonseparate"
java -Xmx40g -ea -cp ${MAINCP} distEA.distEADIVERAnalysis \
	"$query" \
	"$INDIR" \
	"$NT" \
	"-separate" \
